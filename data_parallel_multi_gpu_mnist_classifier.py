...
class Trainer:
    ...
    def _run_epoch(self, epoch):
        ...
        print(f"[GPU {self.device}] Epoch {epoch} | Batchsize: {b_sz} | Steps: {len(self.train_data_loader)}")
        i = 0
...
def main(device: str,
    ...
    trainer = Trainer(model, train_data_loader, test_data_loader, optimizer, device, save_every)
    print(profiler)
    if profiler is True:
    ...
    check_accuracy(trainer.model, test_data_loader, device)
    PATH = './mnist_dp_mult_gpu.pth'
...
if __name__ == "__main__":
    ...
    main(device, args.total_epochs, args.save_every, args.batch_size * world_size, input_size, num_classes, args.profiler)
